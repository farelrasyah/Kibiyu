...
import 'package:kibiyu/page_nineteen.dart';
import 'package:kibiyu/page_seventeen.dart';
import 'home_page.dart';


class PageEightth extends StatefulWidget {
  @override
  _PageEightthState createState() => _PageEightthState();
}

class _PageEightthState extends State<PageEightth> with SingleTickerProviderStateMixin, WidgetsBindingObserver {
  ...
  void initState() {
    ...
    playBackgroundAudio('assets/senang.mp3'); // Memulai musik latar
    playVoiceOverAudio('assets/hal_18.mp3'); // Memulai voice over
  }
  ...
  void toggleAudio() {
    if (isPlaying || isVoiceOverPlaying) {
      ...
      playBackgroundAudio('assets/senang.mp3');
      playVoiceOverAudio('assets/hal_18.mp3');
      setState(() {
  ...
  Route _createHomePageRoute() {
    return PageRouteBuilder(
      ...
      transitionDuration: Duration(milliseconds: 300), // Ganti dengan nama class halaman home Anda
      transitionsBuilder: (context, animation, secondaryAnimation, child) {
        var begin = Offset(0.0, 1.0);
        var end = Offset.zero;
        var curve = Curves.easeInOutCubic; // Kurva yang memberikan efek meluncur yang lembut
  ...
  Route _createRouteBack() {
    return PageRouteBuilder(
      ...
      pageBuilder: (context, animation, secondaryAnimation) => PageSeventh(),
      transitionsBuilder: (context, animation, secondaryAnimation, child) {
  ...
  Route _createRouteForward() {
    return PageRouteBuilder(
      transitionDuration: Duration(milliseconds: 800),
      pageBuilder: (context, animation, secondaryAnimation) => PageNineth(),
      transitionsBuilder: (context, animation, secondaryAnimation, child) {
        var begin = Offset(1.0, 0.0);
        var end = Offset.zero;
        var curve = Curves.easeInOutQuint;
